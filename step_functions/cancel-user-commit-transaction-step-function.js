...
const con = mysql.createConnection({
	...
	eventbase : config.dbconf.RDS_DATABASE_2
});
...
module.exports.submit = async (event, context) => {
  ...
  var res = await functions.postCreditCustomerCancel(event, con);
  return res.insertId > 0 ? 'authorisation reversed' : 'reversal failed';
  // return res
}
